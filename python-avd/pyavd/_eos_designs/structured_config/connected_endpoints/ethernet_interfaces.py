...
class EthernetInterfacesMixin(Protocol):
    ...
    def _get_ethernet_subinterface_cfg(
        ...
        if (dot1q_client_vlan := subinterface.encapsulation_vlan.client_dot1q or subinterface.number) > 4094:
            msg = (
                f"'encapsulation_vlan.client_dot1q' must be set for '{adapter._internal_data.context}.subinterfaces[number={subinterface.number}]'"
                " since the subinterface number is above 4094."
            )
            raise AristaAvdInvalidInputsError(msg, host=self.shared_utils.hostname)
        ...
        # EVPN A/A
        # The channel group is not relevant for these subinterfaces but included in the auto hash for consistency.
        dummy_channel_group_id = int("".join(re.findall(r"\d", adapter.switch_ports[0])))
        if (
